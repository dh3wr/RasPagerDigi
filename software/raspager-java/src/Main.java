...
public class Main {
	...
	public static RadioComm radioComm;
	...
	public static void parseArguments(String[] args) {
		...
		if(!configfile.equals("")) {
			...
			System.out.println("Damit der FunkrufSlave ohne GUI gestartet werden kann, muss eine Konfigurationsdatei angegeben werden!");
			System.exit(1);
		}
	...
	public static boolean initRadioComm() {
		if(radioComm == null) {
			try {
				radioComm = new RadioComm(log);
			} catch (FailedRadioSetupException e) {
				log("startServer # FailedRadioSetupException!", Log.ERROR);
				
				if(mainWindow != null) {
					mainWindow.showError("Server starten", "Beim Initialisieren des Senders ist ein Fehler aufgetreten!");
				}
		...
		if(mainWindow != null) {
			...
			if(!s.equals("")) {
				try {
					...
					log("stepWidth # Keine gültige Schrittweite!", Log.ERROR);
				}
			}
		}
	...
	public static void main(String[] args) {
		...
		System.out.println("RasPager - Version " + VERSION + " by Christian Jansen\nBased on FunkrufSlave V1.0 by Ralf Wilke and Michael Delissen");
